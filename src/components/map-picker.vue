 ...
 * @LastEditors: Hevin
 * @LastEditTime: 2020-05-21 18:20:46
-->
<template>
  <el-dialog
    v-if="isDialog"
  ...
  >
    <div id="mappicker-container" :style="{ height: mapHeight + 'px' }" />
    <div v-if="!readonly" id="mappicker-input">
      <table>
        <tr>
          <td>
            <label>按关键词搜索：</label>
          </td>
          <td class="column2" v-if="!autoComplete">
            <label />
          </td>
          <td class="column2">
            <label>左击获取经纬度：</label>
        ...
        <tr>
          ...
          <td class="column2">
            <input :value="lnglat" type="text" readonly="true" />
          </td>
        </tr>
      </table>
    </div>
    <span v-if="!hideFooter && !readonly" slot="footer" class="dialog-footer">
      <el-button @click="close">取 消</el-button>
      <el-button type="primary" @click="confirm">完 成</el-button>
    </span>
  </el-dialog>
  <div v-else id="mappicker-container" :style="{ height: mapHeight + 'px' }" />
</template>
<script>
export default {
  name: "map-picker",
